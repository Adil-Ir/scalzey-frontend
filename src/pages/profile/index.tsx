...
const computeProgress = (profile: ReturnType<typeof useUserProfile>["profile"]) => {
  const fields = [
    profile.name,
    profile.email,
    profile.avatarUrl,
...
export const ProfilePage = () => {
  ...
  const [isPublicView, setIsPublicView] = useState(profile.isPublicProfile ?? false);
        ...
        <div className="flex flex-col lg:flex-row lg:items-start gap-4 lg:gap-4">
          ...
          <div className="flex-1 min-w-0 lg:w-[85%]">
            <ProfileHeader
              ...
              onPublicViewChange={(v) => {
                setIsPublicView(v);
                // TODO: persist isPublicProfile via API
              }}
            ...
            <div className="pt-4 md:pt-6">
              <ProfilePublicView showPrivateFields={!isPublicView} />
            </div>
          </div>
        </div>
      )}
